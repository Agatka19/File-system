...
bool is_subfolder(const char *path, const char *subpath) {
    size_t len = strlen(subpath);
    return strncmp(path, subpath, len);
}
...
char *make_common_path(const char *path1, const char *path2) {
    ...
    res = strncpy(res, path1, i);
    res[i] = '\0';

    return res;
}

const char *make_path_between(const char *path1, const char *path2) {
    if(strlen(path1) == strlen(path2)) return "/";
    int i= 0;
    while (path1[i] == path2[i]) {
        ++path1;
        ++path2;
